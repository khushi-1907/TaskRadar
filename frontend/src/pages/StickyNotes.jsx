...
const StickyNotes = () => {
  ...
  const updateNote = async (id, updateData) => {
    try {
      const { data } = await api.put(`/sticky-notes/${id}`, updateData);
  ...
  const handleMouseMove = (e) => {
    ...
    const newX = Math.max(0, Math.min(e.clientX - containerRect.left - dragOffset.x, 
      containerRect.width - 240));
    const newY = Math.max(0, Math.min(e.clientY - containerRect.top - dragOffset.y, 
      containerRect.height - 240));
